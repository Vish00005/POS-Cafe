          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">
                {activeCount} active order{activeCount !== 1 ? 's' : ''} · Auto-refresh every 3s
              </p>
            </div>
          </div>
          ...
          {['pending', 'preparing', 'completed'].map(status => {
            ...
            return (
              <div key={status} className="space-y-3">
                {/* Column header */}
                <div className={`flex items-center gap-2 p-3 rounded-xl border-2 ${cfg.color} bg-slate-900`}>
                  <Icon size={16} className={cfg.badge.split(' ')[1]} />
                  <span className="font-semibold text-white capitalize">{cfg.label}</span>
                ...
                <div className="space-y-3">
                  {statusOrders.length === 0 ? (
                    <div className="text-center py-8 text-slate-600 text-sm">
                      {status === 'completed' ? 'No completed orders' : 'No orders here'}
                    </div>
                  ) : (
                    statusOrders.map(order => (
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between">
                          <div>
                            <div className="font-bold text-white text-sm font-mono">{order.orderNumber}</div>
                            <div className="text-xs text-slate-400">Table {order.tableNumber}</div>
                          </div>
                          <div className="text-right">
                            <div className="text-xs text-slate-500">{getElapsed(order.createdAt)}</div>
                        ...
                        {cfg.next && (
                          ...
                          >
                            {updating === order._id ? 'Updating...' : cfg.nextLabel}
                          </button>
                        )}
                      </div>
                    ))
