...
import { Plus, Minus, Trash2, ShoppingBag, ArrowLeft, Send } from 'lucide-react';
...
const OrderScreen = () => {
  ...
  const addToCart = (product) => {
    setCart(prev => {
      ...
      if (ex) return prev.map(i => i._id === product._id ? { ...i, quantity: i.quantity + 1 } : i);
      ...
      <div className="flex h-screen overflow-hidden slide-in">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-3 p-4 border-b border-slate-800">
            <button onClick={() => navigate('/pos')} className="p-2 hover:bg-slate-800 rounded-xl text-slate-400 hover:text-white transition-all">
              <ArrowLeft size={18} />
            </button>
            <div>
              <h1 className="text-lg font-bold text-white">New Order — Table {tableNumber}</h1>
              <p className="text-xs text-slate-400">Select items to add to order</p>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        {inCart && (
                          <span className="text-xs bg-indigo-600 text-white px-2 py-0.5 rounded-full">×{inCart.quantity}</span>
                        )}
                      </div>
                    </button>
                  );
        ...
        <div className="w-80 border-l border-slate-800 flex flex-col bg-slate-900">
          <div className="p-4 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-auto bg-indigo-600 text-white text-xs px-2 py-0.5 rounded-full">{cart.length}</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {cart.length === 0 ? (
              ...
              cart.map(item => (
                <div key={item._id} className="flex items-center gap-3 bg-slate-800 rounded-xl p-3">
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-white truncate">{item.name}</div>
                    <div className="text-xs text-slate-400">₹{item.price} each</div>
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateQty(item._id, -1)} className="w-6 h-6 bg-slate-700 hover:bg-slate-600 text-white rounded-lg flex items-center justify-center text-sm">
                      <Minus size={12} />
                    </button>
                    <span className="w-7 text-center text-sm text-white font-medium">{item.quantity}</span>
                    <button onClick={() => updateQty(item._id, 1)} className="w-6 h-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg flex items-center justify-center text-sm">
                      <Plus size={12} />
                    </button>
                  </div>
