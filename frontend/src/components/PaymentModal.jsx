...
import { X, Banknote, Smartphone, CreditCard, ArrowRight, Loader2, Clock, CheckCircle2 } from 'lucide-react';
...
const PaymentModal = ({ total, tableNumber, onConfirm, onClose, loading }) => {
  ...
  const [settings, setSettings] = useState({ upiId: '', upiName: 'Smart Cafeteria' });
  ...
  const methods = [
    { id: 'upi',  label: 'UPI',  desc: 'Scan QR to pay instantly',   icon: Smartphone, color: 'from-purple-600 to-indigo-600', ring: 'ring-indigo-500' },
    { id: 'card', label: 'Card', desc: 'Debit / Credit via Razorpay', icon: CreditCard,  color: 'from-blue-600 to-cyan-600',    ring: 'ring-blue-500' },
    { id: 'cash', label: 'Cash', desc: 'Pay at the counter',          icon: Banknote,   color: 'from-emerald-600 to-teal-600', ring: 'ring-emerald-500' },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="glass w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl max-h-[92vh] overflow-y-auto">
        ...
        {/* Handle */}
        <div className="flex justify-center pt-3 pb-1 sm:hidden"><div className="w-10 h-1 bg-slate-600 rounded-full" /></div>
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/50">
          ...
          <button onClick={onClose} className="p-2 hover:bg-slate-700 rounded-xl text-slate-400 hover:text-white"><X size={18} /></button>
        </div>
            ...
            <div className="space-y-3">
              {methods.map(({ id, label, desc, icon: Icon, color, ring }) => (
                <button key={id} onClick={() => handleSelect(id)}
                  ...
                  <div className={`w-12 h-12 rounded-xl bg-linear-to-br ${color} flex items-center justify-center shrink-0`}>
              ...
              {!settings.upiId ? (
                ...
                <>
                  ...
                  <div className="flex items-start gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 text-xs text-yellow-400">
                    ...
                    <span>After paying, tap "I Have Paid" below. Your order will be placed and a cashier will verify your payment before the kitchen prepares it.</span>
                  </div>

                  <button onClick={() => onConfirm('upi')}
                    disabled={loading}
                    ...
                    {loading ? <><Loader2 size={18} className="animate-spin" />Placing Order...</> : <><CheckCircle2 size={20} />I Have Paid ₹{total.toFixed(2)}</>}
                  </button>
                </>
              )}
              <button onClick={() => setStep('select')} className="w-full text-slate-500 hover:text-white text-sm py-2">← Change method</button>
            </div>
          )}
            ...
            <div className="space-y-4">
              {selectedM && (
                <div className="flex items-center gap-3 p-4 bg-slate-800/60 rounded-2xl">
                  <div className={`w-10 h-10 rounded-xl bg-linear-to-br ${selectedM.color} flex items-center justify-center shrink-0`}>
                    <selectedM.icon size={18} className="text-white" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-white">{selectedM.label}</div>
                    <div className="text-xs text-slate-400">{selectedM.desc}</div>
                  </div>
                </div>
              )}
              ...
              <div className="bg-slate-800/60 rounded-2xl p-4 space-y-2">
                <div className="flex justify-between text-sm text-slate-400"><span>Subtotal</span><span>₹{total.toFixed(2)}</span></div>
                <div className="flex justify-between text-sm text-slate-500 text-xs"><span>Service charge</span><span>₹0.00</span></div>
              ...
              {method === 'cash' && (
                <div className="flex items-center gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 text-sm text-yellow-400">
                  <Banknote size={16} /> Pay ₹{total.toFixed(2)} at the counter after receiving your order.
                </div>
              )}
              {method === 'card' && (
              ...
              <button onClick={() => setStep('select')} disabled={loading} className="w-full text-slate-500 hover:text-white text-sm py-2 disabled:pointer-events-none">← Change method</button>
            </div>
          )}
        </div>
      </div>
