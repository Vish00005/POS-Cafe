...
const ProductReviewsModal = ({ product, onClose }) => {
  ...
  useEffect(() => {
    api.get(`/api/v1/review/${product._id}`)
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-4 border-b border-white/5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-white leading-tight uppercase tracking-tight">{product.name}</h2>
              <div className="flex items-center gap-1.5 mt-0.5">
                <div className="flex text-yellow-400">
                  <Star size={10} fill="currentColor" />
                </div>
                <span className="text-[10px] font-bold text-white">{product.avgRating?.toFixed(1)}</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12 px-6">
              ...
              <p className="text-slate-400 text-xs font-semibold italic">No community feedback for this dish yet.</p>
            </div>
          ) : (
            reviews.map((rev) => (
              <div key={rev._id} className="space-y-2 pb-4 border-b border-white/5 last:border-0 last:pb-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] text-slate-500">{formatDate(rev.createdAt)}</span>
                </div>
                <div className="flex gap-0.5 text-yellow-500">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={9} fill={i < rev.rating ? "currentColor" : "none"} strokeWidth={i < rev.rating ? 0 : 2} />
                  ))}
                </div>
                {rev.comment && (
